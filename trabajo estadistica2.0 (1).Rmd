...
f4=formula(Importante ~ Derechos_Propiedad + Libertad_Empresarial + Libertad_Comercial + Libertad_Financiera)
f5=formula(Importante ~ Derechos_Propiedad + Libertad_Empresarial + Libertad_Comercial + Libertad_Financiera + PBI)
```
...
```{r}
###ANOVA
tanova=anova(reg,reg2,reg3,reg4,reg5)
...
library(lmtest)
bptest(modelo)
#el p-value resulta significativo; por lo que se rechaza la h0, la cual señala que el modelo es homocedástico. Se confirma lo que se mostró en el gráfico: el modelo presenta heterocedasticidad.
```
...
##hay dos países que son valores influyentes que corresponden a la fila 37 y 54. Los países son: Luxembourg y Rwanda 
